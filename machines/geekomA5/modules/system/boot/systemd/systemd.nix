{
  systemd.settings.Manager = {
    # NOTE DefaultTimeoutStopSec is caused by `libpod-<id>.scope` and `libpod-conmon-<id>.scope` units delaying the shutdown.
    ...
    # NOTE DefaultTimeoutStartSec is here to make sure no service halts the boot process by taking forever to start
